...
export const get_number_array_from_query_string_factory = <T extends { [K in keyof T]: string }>(query: T) => (key: keyof T): number[] => {
    const param = query[key];
    ...
    const value = param.split(',').map(p => parseFloat(p)).map(p => {
        if (isNaN(p)) {
            throw new Error(`One of the list query param is not number. Key: ${String(key)}, Value: ${param}`);
        }
